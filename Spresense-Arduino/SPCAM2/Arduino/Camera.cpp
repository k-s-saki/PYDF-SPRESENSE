...
// Public : Capture Picture Start.
bool CameraClass::capturePictureStart()
{
  if (is_device_ready())
  {
    if(still_img && still_img->img_buff->is_queued())
    {
	  if (ioctl(video_fd, VIDIOC_TAKEPICT_START, 0) == 0)
  	    return true;
    } 
  }
...
// Public : Capture a Picture.
CamImage CameraClass::capturePicture( )
{
  struct v4l2_buffer buf;
  if (is_device_ready()) 
  {
...
// Public : Capture Picture Stop.
bool CameraClass::capturePictureStop( )
{
  if (is_device_ready())
  {
    if(still_img && still_img->img_buff->is_queued())
