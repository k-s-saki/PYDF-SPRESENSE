...
type
  ...
  private
    ...
    function Serial_RecvStr(aSerial: TBlockSerial; aTimeout: integer;
      out oRecvStr: string; aDebugStr: string = ''): boolean;
    (*
    ...
    *)
    ...
    function Serial_ReadImageTags(aSerial: TBlockSerial;
...
begin
  ...
  ShowMessage(
    'Iniファイルは文字コードANSIで保存してください。変更後の反映にはアプリケーションの再起動が必要です。');
end;
...
var
  ...
  begin
    Result :=
      (DstSize.cx > 0) and (DstSize.cy > 0) and
      (SrcSize.cx > 0) and (SrcSize.cy > 0);
  end;
...
begin
  ...
  try
    ...
    //成功した設定を読みだす
    if FIni.SectionExists('SERIAL') then
    begin
...
begin
  ...
  begin
    ...
    if v[j] > FDnnInfoList[j].Value then
    begin
...
function TFrmMain.Serial_RecvStr(aSerial: TBlockSerial; aTimeout: integer;
  out oRecvStr: string; aDebugStr: string): boolean;
var
  s: string;
begin
  s := FSer.RecvTerminated(aTimeout, RecvEnd{ #13});
  if not Serial_IsError(aSerial, aDebugStr) then
  begin
    oRecvStr := s;
    Result := True;
  end
...
(*
  ...
  begin
    ...
    if Serial_RecvStr(FSer,10, s,'FirstRead') = false then
      exit;
*)
  while Serial_RecvStr(FSer, 10, s, 'FirstRead') do
  begin
    Debug('receive = ' + s);
    Debug('1= ' + StrDump(s));
    s := Trim(s);
...
begin
  ...
  //FrameNo:
  if Serial_RecvStr(aSerial, Timeout_Str, s, 'FrameNo') = False then
  begin
    aSerial.Purge;
    exit;
  end;
  ...
  //ImageType
  if Serial_RecvStr(aSerial, Timeout_Str, s, 'ImageType') = False then
  begin
    aSerial.Purge();
    exit;
  end;
  ...
  //ImageSize
  if Serial_RecvStr(aSerial, Timeout_Str, s, 'ImageSize') = False then
  begin
    aSerial.Purge();
    exit;
  end;
  ...
  begin
    ...
    try
      try
        ...
        if Serial_IsError(aSerial, 'Serial_StreamImage') then
        begin
          aSerial.Purge();
          exit;
        end;
...
begin
  ...
  begin
    ...
    try
      ...
      if Serial_IsError(aSerial, 'Serial_FileImage') then
      begin
        aSerial.Purge();
        exit;
      end;
